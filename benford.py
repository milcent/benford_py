...
class Analysis(pd.DataFrame):
 ...
 following: {0}".format(list(self.confs.keys())))
    ...
    def first_digits(self, digs, inform=True, MAD=True, conf_level=95,\
        ...
        if str(conf_level) not in list(self.confs.keys()):
            raise ValueError("Value of parameter -conf_level- must be one of the following: {0}".format(list(self.confs.keys())))
        ...
        df['Z_test'] = _Z_test(df,N)
        #Populate dict with the most relevant entries
        self.maps[dig_name] = np.array(_inform_and_map_(df, inform, show_high_Z, conf))
    ...
    def last_two_digits(self, inform=True, MAD=False, conf_level=95, show_high_Z = 'pos', limit_n=None, MSE=False, plot=True):
        ...
        #Populate dict with the most relevant entries
        self.maps['L2D'] = np.array(_inform_and_map_(df, inform, show_high_Z, conf)).astype(int)
